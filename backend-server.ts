...
// Health check
app.get('/make-server-5c1c75e3/health', (req: any, res: any) => {
  res.json({ status: 'ok' });
});

// Create order
app.post('/make-server-5c1c75e3/orders', (req: any, res: any) => {
  try {
    ...
    const order: Order = {
      ...
      customer_last_name,
      pickup_type,
      items: items.filter((item: OrderItem) => item.quantity > 0),
...
// Get active orders
app.get('/make-server-5c1c75e3/orders/active', (req: any, res: any) => {
  try {
...
app.get('/make-server-5c1c75e3/orders/completed', (req: any, res: any) => {
  try {
...
app.put('/make-server-5c1c75e3/orders/:id/complete', (req: any, res: any) => {
  try {
    const { id } = req.params;
...
// Get analytics
app.get('/make-server-5c1c75e3/analytics', (req: any, res: any) => {
  try {
